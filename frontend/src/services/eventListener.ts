...
class EventListenerService {
  ...
  private async autoAcceptTransferIfRelevant(
    ...
    // CRITICAL: Only auto-accept if we are the RECIPIENT (toMSP), not the sender
    if (this.nodeMSP !== terms.toMSP) {
      console.log(
        `[EventListener] Transfer ${output.termsId} not directed to us (toMSP: ${terms.toMSP}, ourMSP: ${this.nodeMSP}), skipping auto-accept`,
      );
      return;
    }
    ...
    console.log(terms);
    console.log(
      `[EventListener] Transfer ${output.termsId} is directed to our node (we are the recipient), proceeding with auto-accept`,
    );
